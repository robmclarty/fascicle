...
It's a debugging tool, and nothing about it reaches your source code. You
keep using `filesystem_logger` (or, opt-in, `http_logger`) exactly as you do
today, and the viewer is a separate process that reads what you already
write.
...
developing remotely and can't reach the file, or when a flow runs long
inside a container:
...
`http_logger` drops events on a transport error, so your flow doesn't wait
on the dev tool being up.
...
you first, because the dashboard has no auth and shows whatever is in your
