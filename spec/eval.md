...
7. **Live amplify dogfood verified.** Drove a real `pnpm --filter @repo/example-amplify amplify --rounds 1 --candidates 1 --budget-min 8 --effort low` against `pnpm fascicle-viewer --listen --port 4242` with `AMPLIFY_VIEWER_URL=http://127.0.0.1:4242/api/ingest`. Events streamed live to the browser as the run progressed (header counter, active-span pulse, cost rolling up the tree). Final state matched the on-disk trajectory exactly — 51 events, $0.1725 cost, 16 spans, 0 errors — confirming both transports tee cleanly. Run dir: `examples/amplify/.runs/20260430-215840/`.
