/**
 * otel-grafana: watch an agent's traces and metrics on a Grafana dashboard.
 *
 ...
 * examples/otel-grafana/dashboard.json (Dashboards > New > Import).
...
if (import.meta.url === `file://${process.argv[1] ?? ''}`) {
  const main = async (): Promise<void> => {
    ...
    console.log('and upload examples/otel-grafana/dashboard.json to see them.')
  }
