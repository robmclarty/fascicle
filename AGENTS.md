...
- **Top-level `examples/*.ts` import the published names** `fascicle` / `fascicle/adapters`, not `@repo/fascicle`, so they are copy-pasteable by npm consumers. Wired in-repo via the root `tsconfig.json` `paths` and the `vitest.config.ts` `resolve.alias`. The `examples/*/` subprojects and `packages/*` keep `@repo/*`. The five agent examples still import `@repo/agents` (workspace-private, repo-only).
