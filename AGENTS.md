...
- **Examples import the published surface** `fascicle` / `fascicle/adapters` / `fascicle/mcp` (and the repo-only dev alias `fascicle/agents`), so they are copy-pasteable by npm consumers. Resolved via root `tsconfig.json` `paths` + `vitest.config.ts` `resolve.alias`.
