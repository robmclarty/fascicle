---
title: Native expansion build intent - OpenAI-compatible core, native Ollama, loop hooks, timeouts, otel
status: draft
date: 2026-07-11
author: rob
tags: [engine, providers, native, openai, ollama, otel, loop-hooks, intent, plumbbob]
---

# Native expansion: the OpenAI-compatible core and the loop's missing knobs

The sequel to [`provider-sovereignty-intent.md`](./provider-sovereignty-intent.md)
(shipped 2026-07-11 as the plumbbob build "Demote the AI SDK behind a native
provider seam", 17/17 steps; see `.plumbbob/builds/2026-07-09-demote-the-ai-sdk-behind-a-native-provider-seam/report.md`).
That build left five open questions and a handful of deferred items. This
intent resolves all of them in one final upgrade run. Sources: the predecessor
build's open questions Q1-Q5 (answers settled 2026-07-11, recorded in the
...
([`ai-sdk-v7-upgrade-spec.md`](./ai-sdk-v7-upgrade-spec.md) §4.4), which this
build closes by verdict.
...
plus `--provenance` per [`provenance-publish-spec.md`](./provenance-publish-spec.md).
Background only.
