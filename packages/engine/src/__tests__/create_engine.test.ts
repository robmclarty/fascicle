...
describe('create_engine', () => {
  ...
  it('list_aliases and list_prices return defensive copies', () => {
    ...
    const aliases_before = engine.list_aliases();
    (aliases_before as Record<string, { provider: string; model_id: string }>)['injected'] = {
      provider: 'x',
      model_id: 'y',
    };
    expect(() => engine.resolve_alias('injected')).toThrow(model_not_found_error);

    const prices_before = engine.list_prices();
    (prices_before as Record<string, { input_per_million: number; output_per_million: number }>)[
      'custom:z'
