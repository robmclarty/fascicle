...
13. **`generate` is the sole public entry point for model calls, and only the `ai_sdk` transport module touches the Vercel AI SDK.** No file outside `src/engine/providers/ai_sdk/` imports from `ai` or invokes `generateText` / `streamText` / `streamObject`; `generate.ts`, `tool_loop.ts`, and every other engine file drive turns through the neutral `invoke_turn` seam. Enforced by `rules/no-ai-import-outside-ai-sdk-provider.yml` (inverted from the pre-native wording, where `generate.ts` itself owned the SDK call).
