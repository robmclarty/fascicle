...
**Rule:** each composer file in `src/core/` depends only on `./types` and `./runner` (plus its own siblings within co-located facilities like `scope`). `sequence.ts` does not import `parallel.ts`. `adversarial.ts` does not import `retry.ts`. Sharing happens through the `step<i, o>` value contract, not through cross-composer calls.
...
**Rule:** the public API of the `core` module is the 16 primitives, `run`, `describe`, shared types, typed errors, and `flow_schema` (all surfaced through the `fascicle` umbrella). Adapters (filesystem checkpoint store, filesystem trajectory logger, langfuse, MCP) live in the `adapters` module and surface through `fascicle/adapters`. Internal helpers — span bookkeeping, event buffering, cleanup registry, alias resolution — are not exported from the module barrel. The `#<module>` alias resolves only to that barrel, so internals are unreachable from other modules by construction.
...
**Rule:** the repo is one package that publishes one npm package. The composition, engine, and adapters layers are deep modules under `src/<module>/`, each reached only through its barrel `index.ts` via a `#<module>` import alias — they are not separate workspace packages. The umbrella surface at the `src/` root (`index.ts`, `model_call.ts`) plus the `adapters` module barrel are what `tsdown` bundles into a single `dist/` that publishes as `fascicle` (with the `adapters` barrel surfaced as the `fascicle/adapters` subpath). The `#`-aliases are inlined into the bundle; `ai`, `zod`, and every `@ai-sdk/*` stay external as peer dependencies.

**Why:** the published artifact was always a single bundle, so the multi-package workspace only ever organized dev-time source and enforced boundaries. Its one unique contribution over plain folders was *default-deny* — `@repo/core`'s manifest structurally could not list `@repo/engine`. Everything else it carried (per-package manifests, version lockstep across N files, the `@repo/*` two-name discipline, `noExternal` re-bundling, `check-deps.mjs` dependency-shape audits) was ceremony that existed *because* there were multiple packages. We recover default-deny at *finer* (directory) granularity for less ceremony: barrel-only `#`-aliases make deep imports impossible through the alias channel, `rules/no-cross-module-relative-import.yml` closes the relative-path channel, and `fallow.toml`'s `[[boundaries.rules]]` DAG is an explicit per-layer default-deny graph. The `#<module>` prefix is the internal signal (it resolves only inside this package and never reaches a consumer); `fascicle` / `fascicle/adapters` are the public ones. Revisit multi-package publish only when a user concretely asks for slim installs and the coordination cost is worth paying for at least one named consumer.

### 16. One manifest, one version

**Rule:** the single root `package.json` is the only version-bearing file in the repo. The `/version` skill bumps it in a single atomic commit — one number, one release note, one tag — via `scripts/bump-version.mjs`. There are no source-level version constants and no lockstep set: the published version lives in exactly one place.

**Why:** with one package and one manifest there is nothing to align — no tarballs to reconcile, no per-module constants to keep honest. The version is a single field, so bumping it is a non-decision. Earlier the repo carried `src/{core,engine}/version.ts` literals (surfaced as `core_version` / `engine_version`) kept in lockstep with the manifest; that ceremony existed only because the workspace was once multi-package, and collapsing to a single package retired it. If a layer ever churns meaningfully faster than the rest and warrants its own release cadence, that is the moment to reconsider splitting it back out into a separately-published package — not before.
