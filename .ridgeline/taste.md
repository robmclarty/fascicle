...
**Rule:** the public API of the `core` module is the 16 primitives, `run`, `describe`, shared types, typed errors, and `flow_schema` (all surfaced through the `fascicle` umbrella). Adapters (filesystem checkpoint store, filesystem trajectory logger, langfuse, MCP) live in sibling modules (`stores`, `observability`, future `mcp`) and surface through `fascicle/adapters`. Internal helpers — span bookkeeping, event buffering, cleanup registry, alias resolution — are not exported from the module barrel. The `#<module>` alias resolves only to that barrel, so internals are unreachable from other modules by construction.
...
**Rule:** the repo is one package that publishes one npm package. The composition, engine, observability, and stores layers are deep modules under `src/<module>/`, each reached only through its barrel `index.ts` via a `#<module>` import alias — they are not separate workspace packages. The umbrella surface at the `src/` root (`index.ts`, `adapters.ts`, `model_call.ts`) is what `tsdown` bundles into a single `dist/` that publishes as `fascicle`. The `#`-aliases are inlined into the bundle; `ai`, `zod`, and every `@ai-sdk/*` stay external as peer dependencies.
