...
### 16. Lockstep version, one manifest

**Rule:** the single root `package.json` carries the published version; the literal `version` constants in `src/core/version.ts` and `src/engine/version.ts` (surfaced as `core_version` / `engine_version`) move in lockstep with it. One `/version` skill bumps all three in a single atomic commit — one number, one release note, one tag. `scripts/check-deps.mjs`, `scripts/check-publish.mjs`, and `scripts/bump-version.mjs` share the enumeration via `scripts/lib/lockstep.mjs` so there is exactly one list of version-bearing files in the repo.

**Why:** with one manifest there is nothing to align across tarballs, so versioning is already a non-decision. The two `version.ts` literals exist so the runtime can report its own version without importing JSON; lockstep keeps them honest against the manifest. If a layer ever churns meaningfully faster than the rest and warrants its own release cadence, that is the moment to reconsider splitting it back out into a separately-published package — not before.
